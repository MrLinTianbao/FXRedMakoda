...
@interface CBHongBaoVC ()<UITableViewDelegate,UITableViewDataSource,UITextViewDelegate> {
    UIButton *btn;
    UIButton *btnTwo;
    ...
    NSString *amount; //金额
    NSString *redCount; //红包个数
    NSString *noteText; //备注
}
...
- (void)leftAndRightNavBar {
    ...
    //导航栏加状态栏高度
    CGFloat y = [[UIApplication sharedApplication] statusBarFrame].size.height + [[[UINavigationController alloc] init] navigationBar].bounds.size.height;
    
    self.leftBtn.frame = CGRectMake(15, y-40, 30, 30);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.numText.tag = indexPath.row;
    
    [cell.numText addTarget:self action:@selector(textFieldDidChange:) forControlEvents:UIControlEventEditingChanged];
...
- (void)hairBtnAction {
    ...
    noteText = textView.text;
    _redblock(amount,redCount,noteText);
