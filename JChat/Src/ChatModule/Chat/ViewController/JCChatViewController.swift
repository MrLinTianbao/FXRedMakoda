...
extension JCChatViewController: SAIToolboxInputViewDataSource, SAIToolboxInputViewDelegate {
    ...
    open func toolbox(_ toolbox: SAIToolboxInputView, didSelectFor item: SAIToolboxItem) {
        ...
        case "page:redPackage":
            
                let hbVC = CBHongBaoVC()
                
                hbVC.redblock = {(amount,count,text) in
                    let dic = ["hb_infor":["hb_extra": text ?? "","hb_id": "群id或者个人id加时间戳","hb_money": amount ?? "0","hb_num": count ?? "0","is_group": false,"received_id": "1022群id或者个人id","send_user_id": "用户标识"],"message": "###@@@红包###@@@","type": "hb"] as [String : Any]
                    let data : NSData = try! JSONSerialization.data(withJSONObject: dic, options: []) as NSData
                    let jsonString = NSString(data: data as Data, encoding: String.Encoding.utf8.rawValue)! as String
                ...
                self.present(hbVC, animated: true)
...
extension JCChatViewController: JCMessageDelegate,JCRedMessageDelegate {
...
//        }
    ...
    func message(isRecive: Bool, message: JCMessageType, text: String) {
        
            if let content = message.content as? JCRedContent {
    
                if !content.isRecive {
                    content.isRecive = true
