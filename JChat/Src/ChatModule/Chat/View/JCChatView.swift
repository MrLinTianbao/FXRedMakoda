...
@objc public protocol JCChatViewDelegate: NSObjectProtocol {
    ...
    @objc optional func referenceMessage(message: JCMessageType)
...
//            return .init(top: 8 + 2, left: 10 + 2, bottom: 8 + 2, right: 10 + 2)
    ...
    open func collectionView(_ collectionView: UICollectionView, canPerformAction action: Selector, forItemAt indexPath: IndexPath, withSender sender: Any?) -> Bool {
        ...
        if let _ = message.content as? JCRedContent {
            
            if action == #selector(copyMessage(_:)) || action == #selector(forwardMessage(_:)) || action == #selector(withdrawMessage(_:)) || action == #selector(referenceMessage(_:)) {
                return false
            }
        }
        ...
        if let _ = message.content as? JCTextImageContent {
            if action == #selector(referenceMessage(_:)) {
                return true
            }
        }
        
        if let _ = message.content as? JCMessageImageContent {
            if action == #selector(referenceMessage(_:)) {
                return true
            }
        }
    ...
    @objc func referenceMessage(_ sender: Any) {}
...
//        }
        ...
        if action == #selector(referenceMessage(_:)) {
            delegate?.referenceMessage?(message: message)
        }
